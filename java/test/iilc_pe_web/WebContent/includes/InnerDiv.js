...
var INNERDIV = {
	...
	newInnerDiv : function (id,x,y,w,h,miUrl,titulo,miCSS){
		arrayindex=INNERDIV.profundidad.length;
		//**************** comprueba que no haya una ventana con el mismo id ***************************
		if (!INNERDIV.existe(id)){
			...
			//******************************************** barra de la ventana ************************************
				...
				barra.style.backgroundColor = "#3399cc";
			...
			//********************************************* boton de cerrar ***************************************
				...
				enlace.onclick = function(){INNERDIV.cerrar(id);}
				...
				boton.setAttribute("src","cerrar_OLD.gif");
			...
			nuevoDiv.style.zIndex=INNERDIV.cuentaventanas(INNERDIV.profundidad.length)+1;
   ...
   cerrar : function(mi_id){
	...
	total=INNERDIV.profundidad.length;
	//*********** recorre el array de profundidad y elimina tb el objeto del array de profundidad **********
	for (var ind=0; ind<total; ind++) {
		if( mi_id == INNERDIV.profundidad[ind].id){
			var resto = INNERDIV.profundidad.splice(ind,1);
			break;
		}
	}
	...
	existe: function(id){
		...
		for (var ind=0; ind<arrayindex; ind++) {
			if( id == INNERDIV.profundidad[ind].id){
				//alert("ya existe la ventana " + INNERDIV.profundidad[ind].id);
				return(true);
			}
		}
	},
	cargar : function (id,miUrl,params){
			...
			if(INNERDIV.profundidad.length>0){
...
var Drag = {
	...
	start : function(e)
	{
		...
		//*                    hartum@hotmail.com                     *
		//*************************************************************
		o.root.style.zIndex = INNERDIV.cuentaventanas(INNERDIV.profundidad.length)+1;
...
function CargarContenido(targetId,url,params) {
	...
	if (!myConn) alert("Su version de navegador no soporta XMLHTTP para hacer carga dinamica, actualicelo por uno con soporte DOM.");
	var query = function (oXML) { 
		target.innerHTML = oXML.responseText;
		var fader = new Fadomatic(target.parentNode.parentNode, 7, 0, 0,100);
			fader.fadeIn();
	};
