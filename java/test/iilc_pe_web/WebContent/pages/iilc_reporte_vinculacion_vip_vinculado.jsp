...
function openFile(tipo){
	if(hayResultados){
		...
		ventana = window.open("consultaListadoConduccionAction.do?method=espera&tipo="+tipo+"&origenVinculacion=4" ,"",opciones);
	}
	else{
...
function abrirFiltro(tipo){ 
	...
	var url = "consulta2.do?method=filtroPerfil";
...
function updateBotonesFiltro(){
	if(document.forms[0].perfil.value==null || document.forms[0].perfil.value=="-1"){
...
function salir(){
	...
	var contexto = document.images[0].src.substring(0,document.images[0].src.indexOf('images', 0));
...
function buscar(){
	...
	var contexto = document.images[0].src.substring(0,document.images[0].src.indexOf('images', 0));
	window.location = contexto+"reporteVinculacionVIPVinculado.do?perfil="+document.forms[0].perfil.value;
...
<html:form action="/reporteVinculacionVIPVinculado.do" >
...
<table width="100%" >
	 <tr> 
		 <td  width="40%" align="left" >
		 		<table width="100%" >
		 			...
		 			<td>
		 				<input type="button" class="botonFiltro" id="btnPerfil" value="Todos" onclick="abrirFiltro(1);" />
		 			</td>
		 			</tr>
		 		</table>
		 </td>
 ...
 <div align="left"	style="width: 100%" >	
 		<display:table name="reporteVinculado"    id="lista" requestURI="/reporteVinculacionVIPVinculado.do" pagesize="30"  summary="false"  width="60%" class="grilla"  >
 			...
 			<display:column    title="Perfil" property="perfil"   class="cabecera"   />
