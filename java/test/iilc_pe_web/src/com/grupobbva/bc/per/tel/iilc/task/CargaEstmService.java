...
public class CargaEstmService {
	...
	public void ejecutarCarga()
	{
		...
		try {
			...
			Tiilc_adm_carga_archivo bean=iCargaDAO.obtenerCargaDelDia(Constantes.CODIGO_ESTM, fecha);
			if(bean!=null && bean.getId_estado()==Constantes.ID_ESTADO_CARGA_PENDIENTE){ 
				
				mensajero.enviarCorreo(Constantes.CODIGO_ESTM,Constantes.ESTADO_CARGA_INICIO, fecha);
				iCargaDAO.lanzarCargaEstm(bean.getId_registro());    
				...
				if(bean.getArchivo_carga()==Constantes.ESTADO_ARCHIVO_CARGA_EXITO){
					mensajero.enviarCorreo(Constantes.CODIGO_ESTM,Constantes.ESTADO_CARGA_FIN, fecha);
				}else{
					mensajero.enviarCorreo(Constantes.CODIGO_ESTM,Constantes.ESTADO_CARGA_ERROR, fecha);
				}
