...
@Component
public class CargaVarioferService {
	...
	public void ejecutarCarga()
	{
		...
		try {
			...
			Tiilc_adm_carga_archivo bean=iCargaDAO.obtenerCargaDelDia(Constantes.CODIGO_VARIOFER, fecha); 
			if(bean!=null && bean.getId_estado()==1){
				mensajero.enviarCorreo(Constantes.CODIGO_VARIOFER,Constantes.ESTADO_CARGA_INICIO, fecha);
				iCargaDAO.lanzarCargaVariofer(bean.getId_registro());
				...
				fecha= Utilitario.dosCeros(hoy.get(Calendar.DAY_OF_MONTH)) + "-"
					...
					+ Utilitario.dosCeros(hoy.get(Calendar.HOUR_OF_DAY)) + ":"
					+ Utilitario.dosCeros(hoy.get(Calendar.MINUTE));
				if(bean.getArchivo_carga()==1){
					mensajero.enviarCorreo(Constantes.CODIGO_VARIOFER,Constantes.ESTADO_CARGA_FIN, fecha);
				}else{
					mensajero.enviarCorreo(Constantes.CODIGO_VARIOFER,Constantes.ESTADO_CARGA_ERROR, fecha);
				}
