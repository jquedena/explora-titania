...
public class LogroConsultaAction extends ComunAction {
	...
	@Actions( { @Action(value = "/iniciarConsultaSemanalMensual", results = {
			@Result(name = "success", type = "tiles", location = "consultaLogro"),
	...
	@Actions( { @Action(value = "/procesarConsultaLogros") })
	public void procesarConsultaLogros() throws Exception {
		try {
			...
			this.getConsultaLogroService().cargarConsultaLogros(
					this.getRubroLogroFiltro());

			listaRubros = this.getConsultaLogroService().getListaRubros();
			listaOficinasGestores = this.getConsultaLogroService()
					.getListaOficinasGestores();
				...
				for (OficinaGestor objOficinaGestor : listaOficinasGestores) {
					for (Rubro objRubro : listaRubros) {
						...
						for (Semaforo objSemaforo : listaSemaforo) {
							if (porcentaje >= objSemaforo.getRango1()
									&& porcentaje <= objSemaforo.getRango2()) {
								objLogroDetalle
										.setColor(objSemaforo.getColor());
								break;
							}
						}
	...
	@Actions( { @Action(value = "/exportarExcelConsultaLogro", results = {
			@Result(name = "success", type = "stream", params = {
					"contentType", "application/vnd.ms-excel", "inputName",
					"excelStream", "contentDisposition",
					"attachment;filename=\"formConsultaLogros.xls\"",
					"bufferSize", "1024" }),
			@Result(name = "error", type = "tiles", location = "panel") }) })
	public String exportarExcelConsultaLogro() throws Exception {
		try {
			...
			DocumentoExcel doc = new DocumentoExcel(super.getServletContext()
					.getRealPath("/rpt/template/formConsultaLogros.xls"));

			doc.setContentValue(6, 2, exportar[0]);
			doc.setContentValue(6, 7, exportar[1]);
			doc.setContentValue(7, 2, exportar[2]);
			doc.setContentValue(7, 7, exportar[3]);
			doc.setContentValue(8, 3, exportar[4]);
				...
				for (Rubro objRubro : listaRubros) {
					...
					for (LogroMeta objLogroMeta : listaLogros) {
						if (objLogroDetalle.getCodigo().equals(objLogroMeta.getCodigo())
								&& objLogroDetalle.getIdRubro() == objLogroMeta.getIdRubro()) {
			...
			HSSFCellStyle styleTitle = doc.crearStyleDynamic(HSSFCellStyle.ALIGN_CENTER, HSSFColor.BLUE_GREY.index, 83,142,213);
			
			for (int x = 0; x < listaOficinasGestores.size(); x++) {
				doc.setContentValue(rowIndexGestores, 0, this
						.getConsultaLogroService().getListaOficinasGestores()
						.get(x).getDescripcion(), styleTitle);
				...
				for (int y = 0; y < listaRubros.size(); y++) {
					...
					if (x == 0) {
						doc.setContentValue(rowIndexrubros, col, col + 1, this
								.getConsultaLogroService().getListaRubros()
								.get(y).getDescrubro(), styleTitle);

						doc.setContentValue(rowIndexrubros + 1, col, "Metas", styleTitle);
						doc.setContentValue(rowIndexrubros + 1, col + 1, "Logros", styleTitle);
						col = col + 2;
					}

					ld = new LogroDetalle();
					ld = obtenerLogroDetalle(listaLogroDetalle,
							listaOficinasGestores.get(x).getCodigo(),
							listaRubros.get(y).getId());
			...
			this.logger.error("exportarExcelConsultaLogro", ex);
			return ERROR;
		}
	...
	public LogroDetalle obtenerLogroDetalle(
			List<LogroDetalle> listaLogroDetalle, String codigoGestor,
			Long idRubro) {
	...
	public void setConsultaLogroService(
			ConsultaLogroService consultaLogroService) {
