...
public class DAOContratoTemporalImpl extends DAOGenericoImpl<ContratoTemporalDto> implements DAOContratoTemporal<ContratoTemporalDto>{
	...
	public List<ContratoTemporalDto> listaContratosTemporalesPorCliente(String codigoCliente) {
		...
		try {
			...
			query += "'" + codigoCliente +"' and not exists (select c.codigoContrato from ContratoDto c, OficinaSolicitudDto os, SolicitudDto s where c.oficinaSolicitudDto.id = os.id and os.solicitudDto.id = s.id and s.tramiteSolicitudDto.id not in (1021, 1022, 1018) and s.codigoCliente = '"+codigoCliente+"' and c.codigoContrato = e.codigoContrato)" +
