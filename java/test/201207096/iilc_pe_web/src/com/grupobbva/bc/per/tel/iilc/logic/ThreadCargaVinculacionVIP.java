...
public class ThreadCargaVinculacionVIP extends Thread {

	static Logger logger = Logger.getLogger(ThreadCargaVinculacionPH.class);
	private long idRegistroCargaVIP;
	...
	public void run() {
		...
		try {
			...
			String query = "{ call IILC.PIILC_CARGA_VINCULACION.sp_carga_vinculacion_vip(?,?) }";
			stmt = con.prepareCall(query);
			stmt.registerOutParameter(2, Types.INTEGER);
			stmt.setLong(1, this.getIdRegistroCargaVIP());
			stmt.execute();
			...
			e1.printStackTrace();
			logger.error(" ThreadCargaVinculacionVIP :" + e1.getMessage());
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
	...
	/**
	 * @return Returns the idRegistroCargaVIP.
	 */
	public long getIdRegistroCargaVIP() {
		return idRegistroCargaVIP;
	}
	/**
	 * @param idRegistroCargaVIP The idRegistroCargaVIP to set.
	 */
	public void setIdRegistroCargaVIP(long idRegistroCargaVIP) {
		this.idRegistroCargaVIP = idRegistroCargaVIP;
	}
