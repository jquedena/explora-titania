...
<%
Vector d = (Vector)request.getAttribute("listaMostrarEditar");
ListabusAllTerritorio bean = null;
String colorfila = "";

if(d != null && d.size() > 0) {	
	bean = (ListabusAllTerritorio)d.elementAt(0); 
}
%>

<script language="javascript">
	window.addEvent('domready', function(){
		var color = new ColorPicker('colorFondo', 'colorFondo_div', {color:'<%=bean.getColorFondo() %>'}, 'mantenimientoTerritorioAction.do?method=popupSelectorColor');	
	});	
</script>
...
function enviar(){
	...
	var ValorEst = document.forms.est.value;
	...
	document.forms.estado.value=ValorEst;
	document.forms.action="mantenimientoTerritorioAction.do?method=editar";
...
function inicio(){
	var estado = document.forms.est;
	var valor = "<%=bean.getEstado() %>";
	var indice = -1; 
	
	if(valor != "") {
		for(var i = 0; i < estado.options.length; i++) {
			if(estado.options[i].value == valor) {
				indice = i;
				break;
			}	
		}		
...
<div align="center"><strong>EDITAR TERRITORIO</strong></div><br />			
	<table cellspacing="0" cellpadding="0" align="center">
	  <tr>
	    <td class="colorfila1" width="60">Codigo:</td>
	    <td class="colorfila1" align="left"><input name="cod" type="text" value="<%=bean.getCodTerritorio() %>" maxlength="4"  disabled="disabled" class="cajatexto"/>  </td>
	  </tr>
	  <tr>
	    <td class="colorfila2">Nombre:</td>
	    <td class="colorfila2" align="left">
			<input name="nom" type="text" value="<%=bean.getNomTerritorio() %>" size = "35" maxlength="99" class="cajatexto" /></td>
	  </tr>  
	  <tr>
	    <td class="colorfila1" valign="top">Estado:</td>
	    <td align="left">
				<select id="est" name="est" class="combo">
					<option value="0">Activo (0)</option>
					<option value="1">Inactivo (1)</option>
				</select>
		</td>
	  </tr>
	   <tr>
	    ...
	    <td class="colorfila2" align="left">
	    	<textarea class="textocelda" id="mensaje" name="mensaje" rows="15" cols="80" style="width: 80%">						
						<c:out escapeXml="false" value="<%=bean.getMensaje() %>"/>
	    	</textarea>
		</td>
	  </tr>
	   <tr>
	...
	<tr >
	    ...
	    <td>
	    	<input type="button" name="Cancelar" value="Cancelar" onClick="history.back()" class="botonnew">
	    </td>
	    </tr></table>
	    </td>
	</tr>
