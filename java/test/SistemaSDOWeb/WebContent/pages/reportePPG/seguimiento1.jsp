...
function cargar(){
		...
		document.forms.action="seguimientoAction.do?method=reporteSeguimiento1";
...
function grabar(){
	if(valida()){
		...
		document.forms.action="seguimientoAction.do?method=reporteSeguimiento1Grabar";
...
<table align="center" width="100%" height="%" cellspacing="0"
	...
	<tr>
		...
		<table cellspacing="0" cellpadding="0">
			<tr>
				<td class="bls">&nbsp;</td><!--<img src="<%=request.getContextPath()%>/images/esq_izq.jpg">-->
				<td width="200" class="textocentradoblanco">PPG / SEGUIMIENTO 1</td>
				<td class="brs">&nbsp;</td><!--<img src="<%=request.getContextPath()%>/images/esq_der.jpg">-->
			</tr>
		</table>
		</td>
	</tr>
	<tr>
		...
		<table cellspacing="0" cellpadding="0" align="center">
			...
			%> <select name="gestor" class="combo"
					onChange="cargar();">
		...
		<table id="accionesConcretadas" cellspacing="0" cellpadding="0" align="center" class="DataGrid" width="1000px">
			...
			<tr>
				...
				<td><%=bean.getNomEpigrafe().trim()%>&nbsp;</td>
				...
				<td align="right">&nbsp;<%if (sw_perfil || bean.getEstado() == 0) {%>
				<%=bean.getNumeroGestiones1()%> <%} else {%> 
				
				<input style="text-align: right" type="text"
					class="textBlanco Text Gestionados" maxlength="5"
					name="txtNumGestiones_1_<%=bean.getCodAccion() %>_<%=bean.getCodEpigrafe() %>"
					value="<%=bean.getNumeroGestiones1() %>" onKeyPress="ingresoEnteros(event);"> <%}%></td>
				<td align="right">&nbsp; <%if (sw_perfil || bean.getEstado() == 0) {%>
				<%=bean.getNumeroConcretados1()%> <%} else { %> <input
					style="text-align: right" align="right" type="text"
					class="textBlanco Text" maxlength="4"
					name="txtNumConcretados_1_<%=bean.getCodAccion() %>_<%=bean.getCodEpigrafe() %>"
					value="<%=bean.getNumeroConcretados1() %>"
					onKeyPress="ingresoEnteros(event);"> <%}%></td>
				<td align="right">&nbsp; <%if (sw_perfil || bean.getEstado() == 0) {%>
				<%=bean.getMonto1()%> <%} else { %> <input
					style="text-align: right" align="right" type="text"
					class="textBlanco Text" maxlength="9"
					name="txtMonto_1_<%=bean.getCodAccion() %>_<%=bean.getCodEpigrafe() %>"
					value="<%=bean.getMonto1() %>"
					onKeyPress="ingresoEnterosN(event);"> <%}%></td>
			</tr>
			<%}
			if (i == a.size() - 1) {%>
			<tr>
		...
		<table id="esfuerzosConcretados" cellspacing="0" cellpadding="0" align="center" class="DataGrid" width="1000px">
			<tr>
				<th colspan="3" width="240">PRIORIDAD Y OBJETIVOS DE LA OFICINA</th>
				<th colspan="3">ACCIONES CONCRETAS</th>
				<th colspan="3">TOTAL</th>
			</tr>
			<tr>
			...
			<tr>
				...
				<td align="right"><%=bean.getNumeroCliente()%>&nbsp;</td>
				<td align="right"><%=bean.getCompGestor()%></td>
				<td align="right">&nbsp; <%if (sw_perfil || bean.getEstado()==0 || (bean.getNomAccion() != null && bean.getNomAccion().equals("Subtotal"))) {%>
				<%=bean.getNumeroGestiones1()%> <%} else {%> <input
					style="text-align: right; width:50px;" align="right" type="text"
					class="textBlanco Text Gestionados" maxlength="5"
					name="txtNumGestiones_2_<%=bean.getCodAccion()%>_0"
					value="<%=bean.getNumeroGestiones1() %>"
