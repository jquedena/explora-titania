...
* Agregar validacion si se ingresa un numero mayor a 9999 en la caja de texto "Compromiso Gestor"€ emitir el siguiente mensaje de alerta "El monto ingresado debe corresponder a miles de soles"€ y debe permitir grabar.
* En el momento que el gestor seleccione un epígrafe se debe validar si se registraron objetivos, en caso se hayan registrado se mostrara la pantalla de ingreso de acciones, caso contrario en la siguiente pantalla se emitira el mensaje No hay objetivos para este epigrafe, comunicar al Gerente de Oficina".
*/
...
function grabar(codOficinaCompleto, codOficina, codGestor, codAnho, codMes, nomEpigrafe, prioridad, restoOficina, metaGestor, codEpigrafe){
	...
	document.forms.action="reportePPGAction.do?method=nuevaAccion";
	var resp = confirm("\u00BFDesea guardar los cambios?");    		
	if(resp) {
    	document.forms.submit();
   }
}
function volver(codOficinaCompleto, codOficina, codGestor, codAnho, codMes, nomEpigrafe, prioridad, restoOficina, metaGestor, codEpigrafe){
...
<table align="center" width="100%" height="%" cellspacing="0" cellpadding="0">
	...
	<tr>
		<td style="border: 2px solid #264971;padding:20px;" valign="top" height="%" align="center">
			...
			<!-- **** INICIO RESTO DE CÃ“DIGO DE LA PÃGINA-->
<TABLE align="center" >
		<!--TR>
			<TD colspan=2><font class="Titulo2">&nbsp;PPG</font></TD>
		</TR-->
		<TR>
			<TD>OFICINA</TD>
...
<input name="nombreAccion" type="text" value="<%=descAccion %>"  size=70 maxlength="80" class="cajatexto" onKeyPress="return SoloLetras()"/> </TD>
		...
		<TR>
			...
			<TD><input type="text" name="numeroClientes" size=10 maxlength="4" value="<%=numeroCliente %>" onKeyPress="return validKey(INT_PATTERN);" class="cajatexto"></TD>

			<TD class="FondoAzul8" width="200">Compromiso Gestor<br>&nbsp;(Monto en Miles) &nbsp;/&nbsp;N&uacute;mero</TD>
			<TD colspan=2><input type="text" name="compGestor" size=10 maxlength="10" value="<%=compromisoGestor %>" onKeyPress="return validKey(INT_PATTERN_NEGATIVO);" class="cajatexto"></TD>
		</TR>
	</TABLE>
	<TABLE align="center">
<tr><td align="center">
	<c:if test="${sessionScope.gestor.codigoPerfil != 'CON'}">
	<input type="button" name="Grabar" onClick="grabar('<%=codOficinaCompleto %>', '<%=codOficina %>', '<%=codGestor %>', '<%=codAnho %>', '<%=codMes %>', '<%=nomEpigrafe %>', '<%=prioridad %>', '<%=restoOficina %>', '<%=metaGestor %>','<%=codEpigrafe %>');" value="Grabar">
	</c:if>&nbsp;
	<input type="button" name="Volver" onClick="volver('<%=codOficinaCompleto %>', '<%=codOficina %>', '<%=codGestor %>', '<%=codAnho %>', '<%=codMes %>', '<%=nomEpigrafe %>', '<%=prioridad %>', '<%=restoOficina %>', '<%=metaGestor %>','<%=codEpigrafe %>');" value="Volver">
</td></tr>


	</TABLE>

<!-- **** FIN RESTO DE CÃ“DIGO DE LA PÃGINA-->
