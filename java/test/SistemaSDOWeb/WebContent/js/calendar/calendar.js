...
var Calendar = new Class({
    initialize: function(el,open,Config) {
      ...
      lng['nl'] = {
      	...
      	first: 1 // Monday
			...
			lng = (!lng[nav])? lng['es'] : lng =  lng[nav] ;
      /* configuration */
      if (!Config)
	      this.config = {
						Lng: lng,
    ...
    create_calendar: function() {
     ...
     if ($('ncalendar')) $('ncalendar').dispose();
      // content div  //
      ...
      this.title.addEvent('click', function (e) {
        ...
        for (var a=(date.getFullYear()-2); a<= (date.getFullYear()+2);a++) {
          var li = new Element('li').appendText(a).injectInside(ul)
          .setProperty('id',a)
          .addEvent('click', function (e) {
            localThis.tbody.dispose();
   ...
   /* everydays */
      ...
      var brea_k; // breaking week
  	  var daycounter = 0;
     	for (var i = 1; i <= (30 +  this.config.Lng.first); i++) {
    		date_s.setDate(i);
		...
		effect: function(div,op) {		 
			(op == 'fade')? div.setStyle('display','none'): div.setStyle('display','');
		},
