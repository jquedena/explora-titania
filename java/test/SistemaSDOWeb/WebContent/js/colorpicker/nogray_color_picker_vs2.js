...
var ColorPicker = new Class({		
		options: {
			color: '#FF0000',
			openOnClick:true,
			colorPickerInterface:'/color_picker_interface.html',
		...
		initialize: function (el, swatch, options, ruta_base){
			this.element = $(el);
			this.swatch = $(swatch);
			this.randomLetters = $randomLetters(8);
			this.setOptions(options);
			this.options.colorPickerInterface = ruta_base;
		...
		showPicker: function(){
			var lnk = this.options.colorPickerInterface+"&color="+this.options.color+"&pre_color="+this.element.value+"&pickerObject="+this.randomLetters;
