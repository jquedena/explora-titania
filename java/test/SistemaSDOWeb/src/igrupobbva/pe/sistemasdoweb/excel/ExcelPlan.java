...
import com.grupobbva.bc.per.tele.sdo.serializable.ReporteExcelPlanificacion;
...
public class ExcelPlan {
	private static Logger log = Logger.getLogger(ExcelPlan.class);	
	
	public boolean generarExcel(Vector datos, String path, String titulo){
		boolean res = false;
		Workbook original = null;
		WritableWorkbook workbook = null;
		ReporteExcelPlanificacion beanLinea;
		int fila;
		
		try {
			log.info("[ExcelPlan :: generarExcel] Inicio");
			...
			//verificaciones
				...
				if (test.exists())
					...
					if (!test.delete()){
						log.info("[ExcelPlan :: generarExcel] Archivo bloqueado : " + path);
...
//			estilos
			...
			label = new Label(1,fila,"Mes", formatoCeldaTitulo); 
			sheet.addCell(label);
			
			label = new Label(2,fila,"A" + (char) 241 + "o", formatoCeldaTitulo); 
			sheet.addCell(label);
			
			label = new Label(3,fila,"Fecha", formatoCeldaTitulo); 
			sheet.addCell(label);
			...
			for(int i=0; i<datos.size(); i++){
				beanLinea = (ReporteExcelPlanificacion)datos.elementAt(i);
				...
				//mes
				label = new Label(1,fila,beanLinea.getMes(), formatoCeldaNormalDerecha); 
				sheet.addCell(label);
				
				//anio
				label = new Label(2,fila,beanLinea.getAnio(), formatoCeldaNormalDerecha); 
				sheet.addCell(label);
				
				//fecha
				label = new Label(3,fila,beanLinea.getFecha(), formatoCeldaNormalDerecha);
				sheet.addCell(label);
			...
			log.info("[ExcelPlan :: generarExcel] Fin");
		} catch (IOException e) {
			log.error("[ExcelPlan :: generarExcel] ");
