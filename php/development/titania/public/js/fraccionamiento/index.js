...
timeContribuyentePredio = function(rowid) {
	...
	if($("#mhresum").val() == undefined || $("#mhresum").val() == null){
        ...
        _post = $.post(path + "fraccionamiento/listarpredio", parameters);
...
function checktodosfracci(){
	monto=0;
	$('.checkfracc').each( function() {			
		if($("input[id=checkfracctodos]:checked").length == 1){
			this.checked = true;
			var check= String(this.checked);
			//alert('Indice es ' + this.id + ' y valor es: ' + check);
			
			monto=parseFloat(monto)+parseFloat(this.value);
			
			console.log(monto);
			
			$("#txtdeudatotalfracc").val(Math.round((monto*100))/100);
			
		} else {
			this.checked = false;
			$("#txtdeudatotalfracc").val(0);
		}
	});
}

function calculamontofracc(id){
	//alert(id);
	if($("input[id="+id+"]:checked").length == 1){
	
		monto=parseFloat(monto)+parseFloat($("#"+id).val());
	}
	else
	{
		monto=parseFloat(monto)-parseFloat($("#"+id).val());
