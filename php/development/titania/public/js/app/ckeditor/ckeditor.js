...
isEditable:function(i){
    var l=this;
    var j=l.getName();
    if(l.isReadOnly()||l.getComputedStyle('display')=='none'||l.getComputedStyle('visibility')=='hidden'||l.is('a')&&l.data('cke-saved-name')&&!l.getChildCount()||f.$nonEditable[j]||f.$empty[j])return false;
    if(i!==false){
        var k=f[j]||f.span;
        return k&&k['#'];
    }
...
setAttribute:(function(){
    ...
    if(c&&(b.ie7Compat||b.ie6Compat))return function(j,k){
        ...
        else if(j=='contenteditable')i.call(l,'contentEditable',k);else i.apply(l,arguments);
        return l;
    };
...
removeAttribute:(function(){
    ...
    if(c&&(b.ie7Compat||b.ie6Compat))return function(j){
        ...
        else if(j=='contenteditable')j='contentEditable';
        i.call(this,j);
    };else return i;
})(),
...
function o(u){
    ...
    return function(y){
        if(w(y))return true;
        if(y.type==3){
            if(c&&x.test(y.getText())&&!v&&!(u&&y.getNext()))v=true;
            else if(y.hasAscendant('pre')||e.trim(y.getText()).length)return false;
        }else if(y.type==1)if(!n[y.getName()])if(!c&&y.is('br')&&!v&&!(u&&y.getNext()))v=true;else return false;
        return true;
    };
...
moveToElementEditablePosition:function(u,v){
    ...
    function x(z,A){
        var B;
        if(z.type==1&&z.isEditable(false))B=z[v?'getLast':'getFirst'](t);
        if(!A&&!B)B=z[v?'getPrevious':'getNext'](t);
        return B;
    };
    ...
    while(u){
        if(u.type==3){
            if(v&&this.checkEndOfBlock()&&w.test(u.getText()))this.moveToPosition(u,3);else this.moveToPosition(u,v?4:3);
            y=1;
            break;
        }
        if(u.type==1)if(u.isEditable()){
            ...
            y=1;
        }else if(v&&u.is('br')&&this.checkEndOfBlock())this.moveToPosition(u,3);
        u=x(u,y);
    }
    return!!y;
},
...
for(v in u)x.elements[v]=r();var y={
    ...
    elements:{$:function(T){var U=T.attributes;if(U){if(U['data-cke-temp'])return false;var V=['name','href','src'],W;
...
j.list={
    listToArray:function(N,O,P,Q,R){
...
function ac(ad){
    if((aa=Z[ad?'getFirst':'getLast']())&&!(aa.is&&aa.isBlockBoundary())&&(ab=O.root[ad?'getPrevious':'getNext'](d.walker.whitespaces(true)))&&!(ab.is&&ab.isBlockBoundary({
        br:1
...
for(var I in D.$listItem)H.elements[I]=G();var J={
    elements:{}
};

for(I in D.$listItem)J.elements[I]=G(true);function K(N){
...
function L(N,O,P){
    ...
    if(Y){
        ...
        if(aa.is('li')){
            ab=aa.getParent();
            if(aa.equals(ab.getLast(q))&&aa.equals(ab.getFirst(q)))aa=ab;
        }
    ...
    var af=ae.next();
    if(af&&af.type==1&&af.getName() in f.$list)C(af);
    O.moveToBookmark(R);
    O.select();
    N.selectionChange(1);
    N.fire('saveSnapshot');
...
j.add('list',{
    init:function(N){
        ...
        N.ui.addButton('NumberedList',{
            label:N.lang.numberedlist,
            command:'numberedlist'
        });
        N.ui.addButton('BulletedList',{
            label:N.lang.bulletedlist,
        ...
        N.on('key',function(Q){
            ...
            if(ab){
                ...
                if(af&&af.type==1&&af.getName() in m&&af.equals(ac)){
                    ae=1;
                    af=W.next();
                }else if(T.checkBoundaryOfElement(ad,2))ae=1;
                if(ae&&af){
                    var ag=T.clone();
...
(function(){
    ...
    j.add('pastefromword',{
        ...
        n.on('paste',function(q){
            ...
            p=(p||'location=no,menubar=no,toolbar=no,dependent=yes,minimizable=no,modal=yes,alwaysRaised=yes,resizable=yes,scrollbars=yes')+',width='+n+',height='+o+',top='+q+',left='+r;var s=window.open('',null,p,true);if(!s)return false;try{
            var t=navigator.userAgent.toLowerCase();if(t.indexOf(' chrome/')==-1){
            s.moveTo(r,q);s.resizeTo(n,o);
            }
            s.focus();s.location.href=m;
            }catch(u){
        ...
        readOnly:1,
        exec:function(p){
            var q,r=p.config,s=r.baseHref?'<base href="'+r.baseHref+'"/>':'',t=b.isCustomDomain();if(r.fullPage)q=p.getData().replace(/<head>/,'$&'+s).replace(/[^>]*(?=<\/title>)/,'$& &mdash; '+p.lang.preview);else{
    ...
    j.add(o,{
        init:function(p){
            m=this.path;
            p.addCommand(o,n);
            p.ui.addButton('Preview',{
                label:p.lang.preview,
                command:o
            });
        }
    });
...
function t(E){
    ...
    I=I.replace(/^[ \t]+|[ \t]+$/g,function(O,P,Q){
        ...
        else if(!P)return e.repeat('&nbsp;',O.length-1)+' ';else return ' '+e.repeat('&nbsp;',O.length-1);
    });
    ...
    for(var O=G.length-1;O>=0;O--){
        ...
        if(!K.checkReadOnly()){
            ...
            if(I)while((P=K.getCommonAncestor(0,1))&&(Q=f[P.getName()])&&!(Q&&Q[H])){
                if(P.getName() in f.span)K.splitElement(P);
    ...
    init:function(E){
        ...
        J=e.addFunction(W);
            ...
            if(c){
                ...
                if(b.gecko&&b.version>=10900||b.opera)Y.$.body.contentEditable=ab;
                else if(b.webkit)Y.$.body.parentNode.contentEditable=ab;else Y.$.designMode=ab?'off':'on';
            },0);
            ab&&b.gecko&&e.setTimeout(C,0,null,E);
            X=E.window=new d.window(X);
            Y=E.document=new g(Y);
            ab&&Y.on('dblclick',function(ag){
                var ah=ag.data.getTarget(),ai={
                    element:ah,
                    dialog:''
                };
...
if(a.document.$.documentMode>=8){
    ...
    var K=[];
    for(var L in f.$removeEmpty)K.push('html.CSS1Compat '+L+'[contenteditable=false]');E.addCss(K.join(',')+'{ display:inline-block;}');
}else if(b.gecko){
    E.addCss('html { height: 100% !important; }');
    E.addCss('img:-moz-broken { -moz-force-broken-image-icon : 1;\tmin-width : 24px; min-height : 24px; }');
}else if(c&&b.version<8&&E.config.contentsLangDirection=='ltr')E.addCss('body{margin-right:0;}');
...
(function(){
    ...
    function q(Q,R){
        ...
        for(var T=0,U=S.length;T<U;T++){
            var V=new h(S[T]);
            if(V.getAttribute('type').toLowerCase()=='text')if(R){
...
function r(Q,R){
    ...
    if(S)Q?S.removeAttribute('aria-invalid'):S.setAttribute('aria-invalid',true);
    if(!Q)if(T.select)T.select();else T.focus();
    R&&alert(R);
    T.fire('validated',{
        valid:Q,
        msg:R
...
if(Q._.removeDialogTabs&&(W=Q._.removeDialogTabs[R]))for(X=0;X<W.length;X++)S.removeContents(W[X]);
if(S.onLoad)this.on('load',S.onLoad);
if(S.onShow)this.on('show',S.onShow);
if(S.onHide)this.on('hide',S.onHide);
if(S.onOk)this.on('ok',function(aq){
    Q.fire('saveSnapshot');
    setTimeout(function(){
        Q.fire('saveSnapshot');
    },0);
    if(S.onOk.call(this,aq)===false)aq.data.hide=false;
});
if(S.onCancel)this.on('cancel',function(aq){
    if(S.onCancel.call(this,aq)===false)aq.data.hide=false;
});
var ag=this,ah=function(aq){
    var ar=ag._.contents,as=false;
    for(var at in ar)for(var au in ar[at]){
...
this.on('cancel',function(aq){
    ah(function(ar){
        if(ar.isChanged()){
            if(!confirm(Q.lang.common.confirmCancel))aq.data.hide=false;
            return true;
        }
    });
},this,null,0);
...
function ai(){
    ...
    aq.sort(function(at,au){
        if(at.tabIndex!=au.tabIndex)return au.tabIndex-at.tabIndex;else return at.focusIndex-au.focusIndex;
    });
    var ar=aq.length;
    for(var as=0;as<ar;as++)aq[as].focusIndex=as;
};
...
function aj(aq){
    ...
    ar[au].focus();
    if(ar[au].type=='text')ar[au].select();
};

this.changeFocus=aj;
function ak(aq){
    var ax=this;
    if(ag!=a.dialog._.currentTop)return;
    ...
    if(ar==9||ar==2228224+9){
        ...
        if(at)e.setTimeout(at.click,0,at);
        else if(ax.fire('cancel',{
            hide:true
        }).hide!==false)ax.hide();
        Z=1;
    }else return;
...
function al(aq){
    if(Y)aq.data.preventDefault(1);
    else if(Z)aq.data.stopPropagation();
};

var am=this._.element;
this.on('show',function(){
    am.on('keydown',ak,this);
    if(b.opera||b.gecko)am.on('keypress',al,this);
});
this.on('hide',function(){
    am.removeListener('keydown',ak);
    if(b.opera||b.gecko)am.removeListener('keypress',al);
    ah(function(aq){
        s.apply(aq);
    });
});
...
this.on('show',function(){
    ...
    if(Q.config.dialog_startupFocusTab&&ag._.pageCount>1){
        ...
        if(au._.editor.mode=='wysiwyg'&&c){
            ...
            if(as)if(as.parentElement&&as.parentElement().ownerDocument==Q.document.$||as.item&&as.item(0).ownerDocument==Q.document.$){
...
if(b.ie6Compat)this.on('load',function(aq){
...
var ao=[],ap=a.dialog._.uiElementBuilders.hbox.build(this,{
    type:'hbox',
...
for(X=0;X<ap.length;X++)this._.buttons[ap[X].id]=ap[X];
};
...
show:function(){
    ...
    if(!(Q.getParent()&&Q.getParent().equals(a.document.getBody())))Q.appendTo(a.document.getBody());else Q.setStyle('display','block');
...
foreach:function(Q){
    var T=this;
    for(var R in T._.contents)for(var S in T._.contents[R])Q.call(T,T._.contents[R][S]);return T;
},
reset:(function(){
    var Q=function(R){
...
commitContent:function(){
    var Q=arguments;
    this.foreach(function(R){
        if(c&&this._.currentFocusIndex==R.focusIndex)R.getInputElement().$.blur();
...
addPage:function(Q){
    var ac=this;
    var R=[],S=Q.label?' title="'+e.htmlEncode(Q.label)+'"':'',T=Q.elements,U=a.dialog._.uiElementBuilders.vbox.build(ac,{
        type:'vbox',
...
return R;
    ...
    this.dialog=Q;
    var S=R.contents;
    for(var T=0,U;U=S[T];T++)S[T]=U&&new z(Q,U);
    e.extend(this,R);
};
...
function A(Q){
    ...
    function X(Z){
        ...
        S.y+=af;
        if(S.x+W[3]<V)ag=-W[3];
        else if(S.x-W[1]>ab.width-aa.width-V)ag=ab.width-aa.width+(U.lang.dir=='rtl'?0:W[1]);else ag=S.x;
        if(S.y+W[0]<V)ah=-W[0];
        else if(S.y-W[2]>ab.height-aa.height-V)ah=ab.height-aa.height+W[2];else ah=S.y;
        Q.move(ag,ah,1);
...
(function(){
    k.dialog={
        uiElement:function(Q,R,S,T,U,V,W){
            ...
            var ah=[];
            for(ae in af)ah.push(ae);aa['class']=ah.join(' ');
            ...
            for(ae in Z)ai.push(ae+':'+Z[ae]);if(R.hidden)ai.push('display:none');
            for(ae=ai.length-1;ae>=0;ae--){
                if(ai[ae]==='')ai.splice(ae,1);
            }
            if(ai.length>0)aa.style=(aa.style?aa.style+'; ':'')+ai.join('; ');
            for(ae in aa)Y.push(ae+'="'+e.htmlEncode(aa[ae])+'" ');Y.push('>',ab,'</',X,'>');
            S.push(Y.join(''));
            ...
            if(typeof R.setValue=='function')this.setValue=e.override(this.setValue,function(al){
                return function(am){
                    al.call(this,R.setValue.call(this,am));
                };
            
            });
        if(typeof R.getValue=='function')this.getValue=e.override(this.getValue,function(al){
            return function(){
    ...
    if(this.accessKeyUp&&this.accessKeyDown&&R.accessKey)M(this,Q,'CTRL+'+R.accessKey);
    var ak=this;
    Q.on('load',function(){
...
hbox:function(Q,R,S,T,U){
    ...
    var V=this._.children=R,W=U&&U.widths||null,X=U&&U.height||null,Y={},Z,aa=function(){
        ...
        for(Z=0;Z<S.length;Z++){
            ...
            if(W){
                if(W[Z])ae.push('width:'+m(W[Z]));
            }else ae.push('width:'+Math.floor(100/S.length)+'%');
            if(X)ae.push('height:'+m(X));
            if(U&&U.padding!=undefined)ae.push('padding:'+m(U.padding));
            if(c&&b.quirks&&V[Z].align)ae.push('text-align:'+V[Z].align);
            if(ae.length>0)ac.push('style="'+ae.join('; ')+'" ');
            ac.push('>',S[Z],'</td>');
        }
        ac.push('</tr></tbody>');
        return ac.join('');
...
vbox:function(Q,R,S,T,U){
    ...
    var V=this._.children=R,W=U&&U.width||null,X=U&&U.heights||null,Y=function(){
        ...
        Z.push('style="');
        if(U&&U.expand)Z.push('height:100%;');
        ...
        for(var aa=0;aa<S.length;aa++){
            ...
            if(W)ab.push('width:'+m(W||'100%'));
            if(X)ab.push('height:'+m(X[aa]));
            else if(U&&U.expand)ab.push('height:'+Math.floor(100/S.length)+'%');
            if(U&&U.padding!=undefined)ab.push('padding:'+m(U.padding));
            if(c&&b.quirks&&V[aa].align)ab.push('text-align:'+V[aa].align);
...
k.dialog.hbox.prototype=e.extend(new k.dialog.uiElement(),{
    getChild:function(Q){
        ...
        if(!Q.splice)Q=[Q];
        if(Q.length<2)return R._.children[Q[0]];else return R._.children[Q[0]]&&R._.children[Q[0]].getChild?R._.children[Q[0]].getChild(Q.slice(1,Q.length)):null;
    }
},true);
        ...
        functions:function(){
            ...
            return function(){
                ...
                for(ab=0;ab<W.length;ab++){
                    if(typeof W[ab]=='function')aa.push(W[ab]);else break;
                }
                if(ab<W.length&&typeof W[ab]=='string'){
                    Y=W[ab];
                    ab++;
                }
                if(ab<W.length&&typeof W[ab]=='number')Z=W[ab];
                var ac=Z==2?true:false;
                for(ab=0;ab<aa.length;ab++){
                    if(Z==2)ac=ac&&aa[ab](X);else ac=ac||aa[ab](X);
                }
                return!ac?Y:true;
            };
...
a.on('instanceDestroyed',function(W){
    if(e.isEmpty(a.instances)){
        var X;
        while(X=a.dialog._.currentTop)X.hide();
        I();
    }
    var Y=W.editor._.storedDialogs;
    for(var Z in Y)Y[Z].destroy();
    });
})();
e.extend(a.editor.prototype,{
...
z.on('contentDom',function(){
    ...
    if(c){
        ...
        F.on('focusin',function(O){
            if(O.data.$.srcElement.nodeName!='BODY')return;
...
F.on('mousedown',function(O){
    if(O.data.$.button==2){
        ...
        if(P.type=='None')K=z.window.getScrollPosition();
    }
    M();
});
...
if(b.ie8)G.on('mouseup',function(O){
    if(O.data.getTarget().getName()=='html'){
        var P=a.document.$.selection,Q=P.createRange();
        if(P.type!='None'&&Q.parentElement().ownerDocument==E.$)Q.select();
    }
});
...
function N(O){
    if(I){
        ...
        if(O&&R&&R.type=='None')if(!P.$.queryCommandEnabled('InsertImage')){
            e.setTimeout(N,50,this,true);
            return;
        }
        var S;
        if(R&&R.type&&R.type!='Control'&&(S=R.createRange())&&(S=S.parentElement())&&(S=S.nodeName)&&S.toLowerCase() in {
            input:1,
            textarea:1
        })return;
        H=R&&Q.getRanges()[0];
...
if(b.webkit)E.on('keydown',function(O){
    var P=O.data.getKey();
    switch(P){
...
e.extend(k.dialog,{
    ...
    textInput:function(u,v,w){
        ...
        var A=function(){
            ...
            for(var C in y)B.push(C+'="'+y[C]+'" ');B.push(' /></div>');
            return B.join('');
        };
